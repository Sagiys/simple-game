...
public class Tp2 extends StaticEntity{
	
	private boolean isClose = false;
	
	public Tp2(Handler handler, float x, float y) {
		...
		if(isClose && handler.getKeyManager().keyJustPressed(KeyEvent.VK_F)) 
			{
				...
				handler.getWorld().getEntityManager().getPlayer().setX(1000); 
				handler.getWorld().getEntityManager().getPlayer().setY(450); 
